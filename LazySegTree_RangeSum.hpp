...
/// @brief ある区間内のすべての値xをax+bに変えたり、ある区間内の要素の総和を取得したりできる。
struct LazySegTree{
    private:
    ...
    vector<pair<ll,tuple<ll,ll,ll>>> tree;//vector({和, 遅延情報{あるかどうか, 乗算値(a), 加算値(b)}})
    ...
    LazySegTree(ll how_many_node, ll value){
        ...
        tree = vector<pair<ll,tuple<ll,ll,ll>>>(2*pow2ll[log2N], {0,{0,0,0}});
        for (ll i = 2*pow2ll[log2N]-1; i >= 1; i--){
            if (i >= pow2ll[log2N]){
                tree[i].first = i-pow2ll[log2N] < N ? value: 0LL;
            }
            else{
    ...
    LazySegTree(ll how_many_node, vector<ll> &A){
        ...
        for (ll i = 2*pow2ll[log2N]-1; i >= 1; i--){
            if (i >= pow2ll[log2N]){
                tree[i].first = i-pow2ll[log2N] < N ? A[i-pow2ll[log2N]] : 0LL;
            }
            else{
                tree[i].first = tree[2*i].first + tree[2*i+1].first;
            }
    ...
    void merge_operation(tuple<ll,ll,ll> &X, tuple<ll,ll,ll> &Y){//操作を合成
        if (get<0>(X) == 0){
            X = Y;
            return;
        }
    ...
    void tell_info(ll index_on_tree, ll depth, pair<ll,ll> &now){
        if (get<0>(tree[index_on_tree].second) == 1){
            if (depth == log2N){
                ...
                tree[2*index_on_tree].first = tree[2*index_on_tree].first*a + b*(now.second-now.first+1)/2;
                tree[2*index_on_tree+1].first = tree[2*index_on_tree+1].first*a + b*(now.second-now.first+1)/2;
    ...
    /// @brief indexにおける閉区間[L,R]にある要素の総和を求める。
    /// @param L 左端
    /// @param R 右端
    /// @param depth 基本的に省略する
    /// @param where 基本的に省略する
    /// @return 総和
    ll range_sum(ll L, ll R, ll depth = 0, ll where = 0){
        ...
        if (completely_covered(L,R,now.first,now.second)){//完全に範囲内なら採用
            return tree[index_on_tree].first;
        }
        tell_info(index_on_tree,depth,now);
        return range_sum(L,R,depth+1,2*where) + range_sum(L,R,depth+1,2*where+1);//一部だけ範囲内なら分裂して再帰
    }
    ...
    void range_update(ll L, ll R, ll a, ll b, ll depth = 0, ll where = 0){
        ...
        if (completely_covered(L,R,now.first,now.second)){//完全に範囲内なら採用
            tell_info(index_on_tree,depth,now);
            tree[index_on_tree] = {tree[index_on_tree].first*a + b*(now.second-now.first+1), {1,a,b}};//遅延情報を記録して更新
            return;
        }
        tell_info(index_on_tree,depth,now);
        range_update(L,R,a,b,depth+1,2*where);//一部だけ範囲内なら分裂して再帰
        range_update(L,R,a,b,depth+1,2*where+1);
        tree[index_on_tree].first = tree[2*index_on_tree].first + tree[2*index_on_tree+1].first;
    }
};