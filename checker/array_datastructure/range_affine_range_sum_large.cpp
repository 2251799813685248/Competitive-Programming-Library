...
#include <SegTree_DynamicLazy.hpp>
#include <modint.hpp>


using mint = constant_modint<998244353>;

using sum = array<mint,2>;//{区間長, 和}
using affine = array<mint,2>;//{a,b}でax+bを表す。
sum op(const sum& a, const sum& b){
    return {a[0]+b[0], a[1]+b[1]};
}
sum mapping(const affine& f, const sum& a){
    return {a[0], f[0]*a[1]+f[1]*a[0]};
}
sum init(const ll l, const ll r){
    return {r-l+1, 0};
}
affine composition(const affine& f, const affine& g){
    return {f[0]*g[0], f[0]*g[1]+f[1]};
}
sum e(){return {0,0};}
affine id(){return {1,0};}

void solve(){
    
    ll N,Q;
    cin >> N >> Q;
    DynamicLazySegTree<sum, affine> T(N, e(), op, init, mapping, composition, id());
    ...
    //T.range_update(0, 5, {3,5});
    //T.range_update(3, 9, {3,7});
    //for (int i = 0; i < 10; i++){
    //    cout << T[i][1] << " \n"[i==9];
    //}
    //auto lower = [&](const sum& s){
    //    return s[1].val >= 82;
    //};
    //print(T.min_right(0, lower));
